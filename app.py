"""
Autism Risk Screening System — Streamlit Cloud Ready
TensorFlow is optional — app runs in full demo mode without it.
"""
...
.disclaimer {
    ...
    padding: 1rem 1.3rem; margin-bottom: 1.2rem; font-size: 0.87rem; color: #4a3800;
}
.kpi {
    border-radius: 14px; padding: 1.4rem 1rem 1.2rem;
    text-align: center; color: white; box-shadow: 0 6px 18px rgba(0,0,0,0.18);
}
.kpi .val { font-size: 2.4rem; font-weight: 800; line-height: 1; }
.kpi .lbl { font-size: 0.75rem; opacity: 0.88; margin-top: 0.3rem;
            text-transform: uppercase; letter-spacing: 0.5px; }
.ind { border-radius: 8px; padding: 0.6rem 1rem; margin-bottom: 0.4rem; font-size: 0.88rem; }
...
.gcam-cap { font-size:0.76rem; text-align:center; font-style:italic; color:#6b7280; margin-top:3px; }
.stButton > button {
...
def predict(model, img_array, key):
    """Predict with TF model or simulate score from image pixels."""
    if model is not None and TF_AVAILABLE:
        try:
            return float(model.predict(img_array, verbose=0)[0][0])
        except Exception:
            pass
    # Demo: derive a pseudo-score from image pixel statistics
    # so different images give different scores (not random each run)
    pixels = img_array[0].flatten()
    score  = float(np.clip(abs(np.mean(pixels)) * 2.5 + np.std(pixels) * 0.8, 0.15, 0.85))
    return score

# ── Grad-CAM ───────────────────────────────────────────────────────────────────
def gradcam_overlay(model, img_array, display):
    if not TF_AVAILABLE or model is None:
        return _fake_heatmap(display)
    try:
        import cv2
        ...
        return cv2.addWeighted(display, 0.55, hm_rgb, 0.45, 0)
    except Exception:
        return _fake_heatmap(display)

def _fake_heatmap(display):
    """Generate a demo heatmap from image luminance when TF unavailable."""
    try:
        import cv2
        gray    = np.mean(display, axis=2).astype(np.float32)
        gray    = (gray - gray.min()) / (gray.max() - gray.min() + 1e-8)
        h, w    = display.shape[:2]
        hm_col  = cv2.applyColorMap(np.uint8(255 * gray), cv2.COLORMAP_JET)
        hm_rgb  = cv2.cvtColor(hm_col, cv2.COLOR_BGR2RGB)
        return cv2.addWeighted(display, 0.55, hm_rgb, 0.45, 0)
    except Exception:
        return display
...
def build_report(predictions):
    ...
    return {
        ...
        "risk_colour": colour,
        "per_image": {
            k: {"label": IMAGE_LABELS.get(k, k),
                "confidence": round(v * 100, 1),
                "flagged": v >= 0.5}
            for k, v in predictions.items()
        },
        "indicators": [BEHAVIORAL_SIGNS[k] for k, v in predictions.items()
                       if v >= 0.5 and k in BEHAVIORAL_SIGNS],
        "timestamp": datetime.datetime.now().isoformat(),
    }

# ── UI Helpers ─────────────────────────────────────────────────────────────────
def conf_bar(label, pct, flagged):
    colour = "#ef4444" if flagged else "#22c55e"
    icon   = "🔴" if flagged else "🟢"
    st.markdown(
        f"<div style='display:flex;align-items:center;gap:10px;margin-bottom:6px;"
        f"font-size:0.86rem'><span style='width:200px;font-weight:500'>{icon} {label}</span>"
        f"<span style='font-weight:700;color:{colour};width:48px'>{pct:.1f}%</span></div>",
        unsafe_allow_html=True)
    st.progress(pct / 100)
...
def main():
    ...
    # Model status banner
    model = load_model()
    has_trained = os.path.exists(MODEL_PATH)

    if not TF_AVAILABLE:
        st.warning(
            "🔬 **Demo Mode** — TensorFlow is not available in this environment. "
            "The app is fully functional — image analysis uses pixel-based scoring. "
            "To enable the full AI model, run `python train.py` and upload `models/autism_model.h5`."
        )
    elif not has_trained:
        st.info(
            "🔬 **Demo Mode** — No trained model found. "
            "Using MobileNetV2 with untrained classification head. "
            "Grad-CAM heatmaps are still generated from the base model features."
        )
    else:
    ...
    st.caption("Upload **one image per category**. Each is analysed independently — scores are averaged for the final result.")
    ...
    for col, (key, icon, name, hint) in zip(cols, IMAGE_CONFIG):
        with col:
            ...
            st.caption(hint)
            uf = st.file_uploader(name, type=["jpg","jpeg","png","bmp","webp"],
            ...
            with col:
                if uploaded_files[key]:
                    ...
                    st.markdown(
                        ...
                        "Not uploaded</div>", unsafe_allow_html=True)
    ...
    with st.spinner("🧠 Analysing images…"):
        pil_imgs, img_arrays, display_arrs = {}, {}, {}
        ...
        predictions = {k: predict(model, arr, k) for k, arr in img_arrays.items()}
    ...
    with k1:
        st.markdown(
            ...
            f'<div class="lbl">Autism Risk Score</div></div>', unsafe_allow_html=True)
    with k2:
    ...
    with k3:
        st.markdown(
            ...
            f'<div class="lbl">Images Analysed</div></div>', unsafe_allow_html=True)
    with k4:
        st.markdown(
            f'<div class="kpi" style="background:linear-gradient(135deg,#f7971e,#ffd200)">'
            f'<div class="val">{n_flagged}</div>'
            f'<div class="lbl">Flagged</div></div>', unsafe_allow_html=True)
    ...
    with right:
        ...
        for sign_key, sign_text in BEHAVIORAL_SIGNS.items():
            ...
            st.markdown(f'<div class="ind {css}">{icon} {sign_text}</div>',
                        unsafe_allow_html=True)
    ...
    if st.session_state.get("show_gcam", True):
        ...
        for col, (key, conf) in zip(gcam_cols, predictions.items()):
            with col:
                overlay = gradcam_overlay(model, img_arrays[key], display_arrs[key])
                st.image(Image.fromarray(overlay.astype(np.uint8)),
                         caption=IMAGE_LABELS.get(key, key), use_container_width=True)
                clr = "#ef4444" if conf >= 0.5 else "#22c55e"
                st.markdown(f'<div class="gcam-cap" style="color:{clr}">'
                            f'Confidence: {conf*100:.1f}%</div>', unsafe_allow_html=True)
    ...
    st.info("📋 **Resources:** Autism Speaks · ASHA · National Autistic Society · NIMH\n\nEarly intervention significantly improves long-term outcomes.")
    st.markdown("---")
